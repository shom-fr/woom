...
* PBS Pro: ``nnodes`` and ``ncpus`` are now merged into a single ``-l select=N:ncpus=X:mpiprocs=X``
  directive, fixing a conflict where PBS Pro rejects ``-l ncpus=X`` alongside ``-l select=...``.
